...
const Icon = ({ type = null, spin = false, url = null, size = null, color: color2 = null }) => {
  const getIconStyle = () => {
    let style = {};
    if (url) {
  ...
  if (type == "url") {
    const className2 = ["mvi-icon-url", spin ? "spin" : ""].join(" ");
    return /* @__PURE__ */ jsxRuntimeExports.jsx("i", { className: className2, style: getIconStyle() });
  }
  const className = ["mvi-icon", "icon-" + type, spin ? "spin" : ""].join(" ");
  return /* @__PURE__ */ jsxRuntimeExports.jsx("i", { className, style: getIconStyle() });
};
const version = "0.0.1";
export {
  obj as Dap,
